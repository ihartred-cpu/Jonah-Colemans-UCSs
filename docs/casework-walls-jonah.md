...
**Both fixes have been applied in this repo's copy** of `scripts/casework-walls/jonah-casework-wall-10-studs-and-stud-dadoes.txt` (marked with `CV2025-PATCH` comments, not part of Jonah Coleman's original) — the `_EDGWP == 0` shape test at line ~30 was swapped for `_SHPEDGCNT > 0`, and the `DY -= TEMP_TopAdjust` line was wrapped in `rprec()`. Neither has been run against a real CV2025 install by this repo; the change is sourced from a corroborated forum report (a Verified Answer, cross-checked byte-exact against the source), not independent verification. The same `_EDGWP == 0` idiom was **not** independently re-checked for other occurrences in the 13-script stack — see `TESTING.md`. A second, unrelated `_EDGWP == 0` line exists at `jonah-casework-wall-10-studs-and-stud-dadoes.txt:90` (`if this._FACEWP > 0 and this._EDGWP == 0 then`) but reads as a different idiom (a normalization guarded by `_FACEWP > 0`, not a shape test) and was deliberately left unchanged.
...
So a null `CABNO` combined with a parent that is neither 10 nor 37 means "freshly pulled from the library," and the script uses that to wipe its own version stamps and start clean. **The codes themselves are unpublished and these are the author's labels, not a lookup table** — see the standing caution about `OBJECT` codes in `standard-ucs.md`. What the pattern establishes is the *technique*: parent object class is usable as a provenance discriminator, letting one script distinguish a copy from a library placement from an edit-in-place.
