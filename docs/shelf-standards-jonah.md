...
That makes this a **third and fourth independent attestation of `OBJECT == 17` meaning a Part** — it already appears in `scripts/visible-part-splits/jonah-visible-part-splits.txt` and `scripts/casework-walls/jonah-casework-wall-12-radius-blank-panel-inset-fix.txt` — and, unusually, one where the author labels the branch himself rather than leaving the code to imply it. **Update, 2026-08-03: `OBJECT == 17` = Part is now independently confirmed by Hexagon's own published `OBJECT` code table, not just empirically observed** — see `cv-object-reference.md` for the full table (and the separate, officially-published `<ObjectType>` keyword lists for `For Each`/`Dim`, a different thing from this numeric `OBJECT` variable). What's genuinely new beyond the confirmation is the **use** of it: a part-name filter that includes operation names yields both levels, and `OBJECT` is the discriminator. That is a cheaper structure than the two-script split the corpus otherwise favours.
