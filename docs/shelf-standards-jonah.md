...
`OBJECT == 17` = Part here too — the fourth attestation in the corpus (alongside `scripts/visible-part-splits/jonah-visible-part-splits.txt` and `scripts/casework-walls/jonah-casework-wall-12-radius-blank-panel-inset-fix.txt`), confirmed by Hexagon's own published `OBJECT` code table, and unusually one where the author labels the branch himself rather than leaving the code to imply it. What's genuinely new here isn't the code value — it's the **use**: a part-name filter that includes operation names yields both levels, and `OBJECT` is the discriminator. That's a cheaper structure than the two-script split the corpus otherwise favours.
