...
Source: `examples/jonah-shelf-standards.txt`, byte-exact. Author: Jonah Coleman — the header names him, and the commented library path `FWP\Hardware Visuals\Shelf Standard` plus the `JCS_` parameter prefix both match his known fingerprint (see `examples/README.md` on prefixes).

**Provenance note, superseded 2026-08-01.** The script originally arrived as a mangled paste (line breaks collapsed into runs of spaces — the forum copy/paste mangling Jonah Coleman himself warns about in `full-docs/nexus-thread-notch-construction-rev12.txt`, "Yep use internet explorer"), filed verbatim at `full-docs/jonah-shelf-standards-rev17-raw.txt` with every citation in this file necessarily "by construct, not by line." The user has since supplied a clean re-copy that preserves real line breaks; the mangled file has been deleted and this file's citations rewritten against the real line numbers below. One correction fell out of re-checking against the clean copy — see the `OBJECT` dispatch section — and every other claim held up unchanged, just with a real line number attached now.
...
**Confirmed literally installed in this shop's own UCS Manager, though currently disabled** — a screenshot of the "User Created Standards" window (see `standard-ucs.md`'s UCS Manager UI section) shows an entry named exactly **"JCS Shelf Standards"**, description "Jonah's Shelf Standards REV 17," with its `Enabled` checkbox unchecked. So this isn't only forum-sourced reference material — it's a real entry in the user's own install, just not currently switched on. Why it's disabled isn't visible from the screenshot; don't assume it means the script is abandoned or superseded without asking.
