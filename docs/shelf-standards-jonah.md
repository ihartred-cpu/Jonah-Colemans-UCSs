...
`for each` is already known to be case-insensitive. This script shows the same is true of identifiers and property names, because it mixes casings for the same thing and works anyway:
...
**Confirmed broken, root cause found, fixed, and confirmed fixed against real CV2025.4 — this repo's Revision 22, 2026-07-31/08-01.** Real click-testing showed the described symptom is not actually what happens: setting `ucs_Operation_Type` to Centerline Route produces a line in the assembly view, but the operation never reaches the CAM Reports/nest list at all — it isn't miscategorized as a dado, it's simply never a real machinable operation. Revision 10's own comment describing "gets dim'd as a dado even if set to route" turns out not to be the actual mechanism, fourteen years and multiple CV majors later. The real root cause is a classic object-type pitfall: the route branch is `dim ShelfStandardDado as new line` — `line` is a non-cutting visual/dimension object in CV's object model, never a machinable type — with its width hardcoded to `ShelfStandardDAdo.DX := 0` (also a stray case typo on the variable name, itself a case-insensitivity artifact, not a second bug). It was never going to nest regardless of what else was right.
...
`REPT` and `SPCNG` are the line-bore repeat count and spacing, so the dado spans exactly the bored range. A part-level `JCS_ShelfStd_Length_Over > 0` overrides it, and the centered modes recover the difference against the original span before re-centering.
...
- **Does CV normalize `Size1` to `Size01`?** If it does, the fallback defect above evaporates. **Permanently declined, not just paused, as of the 2026-08-01 test-plan closeout** — the user explicitly won't test this (lowest-value item in the plan; doesn't change any fix's correctness either way regardless of the answer).
...
## Overall test status (as of this repo, 2026-08-01)
