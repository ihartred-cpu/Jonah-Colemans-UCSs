...
**This script has a forum-reported CV2023 breakage that does not reproduce on this shop's install.** Eric@b*.com, roughly two years before capture: the **drawer stretcher origin moved**, from Top Back Left in CV11 to Bottom Front Right in CV2023, so the extended drawer stretcher now sticks out the left side of the cabinet — no reply to that post is present in the capture, and no newer revision is referenced anywhere in it. Real click-testing on this shop's own CV2025.4 install, with the compensation switch left at its shipped default (off), showed an "entire interior"-sized drawer stretcher staying correctly within the interior frame — it did not stick out the wrong side. Read that precisely: it shows the forum-reported regression is **not universal across CV2025 installs/builds**, not that Eric's report was wrong. The flag stays in the script, still off by default, in case a different CV2025.4 build/config hits the forum symptom; whether the compensation switch itself does anything when turned on remains untested (its own name still says `(untested)`) — only the default (off) path was exercised. If a user on modern CV does hit the drawer-stretcher symptom, check the real cabinet first rather than assuming a workaround is needed by default, but this parameter is still the first thing to point them at.
...
The cabinet-vision skill's `standard-ucs.md` has carried `Cab.Interior.CO` as an **open question** — attested only in one of the user's own scripts, absent from that skill's `internal-part-names.md` and `object-intelligence.md`, guessed at as "most likely the interior core/opening region node." This script settles the mechanics, and the name is settled too: `CO` stands for "Case Open," or "Case Opening," confirmed by the user directly and independently corroborated by a Hexagon Nexus "Ideas" post from Matt Bauman (the same named source as the thread-134912 capture in `source-threads/`), titled "Interior Case Openings (CO) > Provide Meaningful Values to the Z & DZ Parameters (for referencing in UCS)," captured verbatim at `source-threads/nexus-idea-co-z-dz-values.txt`. Lines 93-107:
...
Both halves of the old guess were right — it is the opening region node — but the addressable-collection behaviour, the `PARID` link, and the name itself are what the earlier guess couldn't establish. `Z` and `DZ` on a `CO` are always `0`, unconditionally — a standing Hexagon limitation Bauman's Ideas post is requesting be fixed ("Currently the Z & DZ parameters for interior case openings are both 0"), not a field this script simply didn't happen to exercise (his own workaround confirms it: "I often reference `INTERIOR.CO.X` or `INTERIOR.CO.DX`"). `X`/`DX` (and by extension `Y`/`DY`) are the fields that actually carry real geometry — don't write UCS logic that expects `CO.Z`/`CO.DZ` to hold a real depth or setback value.
